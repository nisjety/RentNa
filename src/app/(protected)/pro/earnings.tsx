...
export default function EarningsScreen() {
  ...
  const summary = useQuery(api.cleanerPro.earningsSummary);
  const payouts = useQuery(api.cleanerPro.listPayouts);

  const loading = summary === undefined || payouts === undefined;
  const stats = !summary
    ? { totalKr: 0, jobsCount: 0, hoursWorked: 0 }
    : period === 'week'
      ? summary.thisWeek
      : period === 'month'
        ? summary.thisMonth
  ...
  const monthlyBars = summary?.monthlyBars ?? [];
  const maxKr = Math.max(1, ...monthlyBars.map((b) => b.kr));
        ...
        <View style={[styles.segmented, { backgroundColor: theme.surfaceMuted }]}>
          {(['week', 'month', 'ytd'] as Period[]).map((p) => (
            <Pressable
              key={p}
              onPress={() => setPeriod(p)}
              style={[styles.segment, period === p && { backgroundColor: theme.surface }]}>
              <Text style={[styles.segmentLabel, { color: period === p ? theme.text : theme.textMuted }]}>
                {PERIOD_LABELS[p]}
              </Text>
            </Pressable>
          ))}
          ...
          <>
            <View style={[styles.heroCard, { backgroundColor: theme.text }]}>
              ...
              <View style={styles.heroRow}>
                ...
                <HeroStat
                  value={
                    stats.hoursWorked > 0
                      ? `${Math.round(stats.totalKr / stats.hoursWorked)} kr/t`
                      : '— kr/t'
                  }
                  label="Snittrate"
                  light
            ...
            <View style={[styles.chartCard, { backgroundColor: theme.surface }]}>
              {monthlyBars.length === 0 ? (
                <Text style={[styles.chartEmpty, { color: theme.textMuted }]}>
                  Ingen historikk enda
                </Text>
              ) : (
                <View style={styles.bars}>
                  {monthlyBars.map((b, idx) => {
                    const isLast = idx === monthlyBars.length - 1;
                    const pct = b.kr / maxKr;
                    return (
                      <View key={`${b.month}-${idx}`} style={styles.barCol}>
            ...
            <View style={styles.payoutList}>
              {payouts.length === 0 ? (
                <View style={[styles.payoutRow, { backgroundColor: theme.surface, justifyContent: 'center' }]}>
                  <Text style={[styles.payoutAmount, { color: theme.textMuted }]}>
                    Ingen utbetalinger enda
                  </Text>
                </View>
              ) : (
                payouts.map((p) => (
                  <View key={p._id} style={[styles.payoutRow, { backgroundColor: theme.surface }]}>
                    <View
                      style={[
                        styles.payoutIcon,
                        { backgroundColor: p.status === 'paid' ? '#E5DFD2' : theme.surfaceMuted },
                      ]}>
                      <Icon
...
function HeroStat({ value, label, light }: { value: string; label: string; light?: boolean }) {
  ...
  return (
    <View style={styles.heroStat}>
      ...
      <Text style={[styles.heroStatLabel, { color: light ? theme.surfaceMuted : theme.textSecondary }]}>
        {label}
      </Text>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  segmented: {
    flexDirection: 'row', marginHorizontal: Spacing.four,
    borderRadius: Radius.lg, padding: 3, marginBottom: Spacing.four,
  },
  segment: { flex: 1, paddingVertical: Spacing.two, alignItems: 'center', borderRadius: Radius.md },
  segmentLabel: { ...Typography.caption, fontWeight: '600' },
  loadingCenter: { paddingVertical: Spacing.eight, alignItems: 'center' },
  heroCard: {
    marginHorizontal: Spacing.four, borderRadius: Radius.xl,
    padding: Spacing.five, marginBottom: Spacing.five, gap: Spacing.two,
  },
  ...
  sectionHeader: { paddingHorizontal: Spacing.four, marginBottom: Spacing.three },
  sectionTitle: { ...Typography.subhead, fontWeight: '600' },
  chartCard: {
    marginHorizontal: Spacing.four, borderRadius: Radius.xl,
    padding: Spacing.four, marginBottom: Spacing.five,
  },
  chartEmpty: { ...Typography.caption, textAlign: 'center', paddingVertical: Spacing.four },
  ...
  payoutList: { marginHorizontal: Spacing.four, borderRadius: Radius.lg, overflow: 'hidden', gap: 1 },
  payoutRow: {
    flexDirection: 'row', alignItems: 'center', gap: Spacing.three,
    paddingHorizontal: Spacing.four, paddingVertical: Spacing.three,
  },
  payoutIcon: { width: 36, height: 36, borderRadius: Radius.sm, alignItems: 'center', justifyContent: 'center' },
  payoutAmount: { ...Typography.bodyMedium },
  payoutDate: { ...Typography.caption, marginTop: 2 },
  payoutStatus: { paddingHorizontal: Spacing.two, paddingVertical: 4, borderRadius: Radius.pill },
