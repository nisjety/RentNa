...
import { getCleanerById, SERVICE_LABEL, TAG_LABEL } from '@/data/mock-cleaners';
import { useTheme } from '@/hooks/use-theme';
import { useRoleStore } from '@/stores/role-store';

const ME = getCleanerById('cl_maja')!;

export default function CleanerProfilScreen() {
  const theme = useTheme();
  const router = useRouter();
  const { signOut } = useAuth();
  const { setActiveRole, reset } = useRoleStore();
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: theme.background }]} edges={['top']}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.profileCard, { backgroundColor: theme.surface }]}>
          <Avatar initials={ME.initials} size={60} tone="taupe" />
          <View style={{ flex: 1 }}>
            ...
            <View style={styles.ratingRow}>
              ...
              <Text style={[styles.rating, { color: theme.text }]}>
                {ME.rating.toFixed(2)} · {ME.reviewCount} vurderinger
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.statsRow}>
          <StatCard value={String(ME.jobsCompleted)} label="Oppdrag" />
          <StatCard value={`${ME.yearsExperience} år`} label="Erfaring" />
          <StatCard value={`${ME.hourlyRateKr} kr`} label="Timerate" accent />
        </View>
        ...
        <View style={[styles.card, { backgroundColor: theme.surface }]}>
          {ME.services.map((s) => (
            <View key={s} style={[styles.serviceRow, { borderBottomColor: theme.divider }]}>
              ...
              <Text style={[styles.serviceLabel, { color: theme.text }]}>{SERVICE_LABEL[s]}</Text>
        ...
        <View style={styles.tagRow}>
          {ME.tags.map((t) => (
            <Pill key={t} label={TAG_LABEL[t]} tone="neutral" />
          ))}
        </View>
        ...
        <View style={[styles.section, { backgroundColor: theme.surface }]}>
          <Row
            ...
            onPress={() => router.push(`/cleaner/${ME.id}`)}
          />
          <Row icon="pencil-outline" label="Rediger bio" />
          <Row icon="camera-outline" label="Endre profilbilde" />
        </View>
        ...
        <View style={[styles.section, { backgroundColor: theme.surface }]}>
          <Row icon="cash-outline" label="Betalingsinformasjon" trailing="Vipps · ****4567" />
          <Row icon="notifications-outline" label="Varsler" />
          <Row icon="shield-outline" label="Personvern og sikkerhet" />
        </View>
